...
vec3 firePalette(float i){
    ...
    float T = 400. + 1300.*i + 1000. * pow(iMusic[0].z, 1.2 + length(iMusic[2].xz)); // Temperature range (in Kelvin).
    vec3 L = vec3(7.4, 5.6, 4.4); // Red, green, blue wavelengths (in hundreds of nanometers).
    L = pow(L,vec3(5.0 - (iMusic[1].x - iMusic[1].y))) * (exp(1.43876719683e5/(T*L))-1.0);
...
float noiseLayers(in vec3 p) {
    ...
    for (int i = 0; i < iter; i++) {
        ...
        p *= 1.2 + length(iMusic[1].xz )/3.  ; // Position multiplied by two.
        t *= 1.5; // Time multiplied by less than two.
        sum += amp; // Sum of amplitudes.
        amp *= 0.2 + length(iMusic[2].xz)/3.; // Decrease successive layer amplitude, as normal.
    }
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // Nebula.
    c *= sqrt(c)*(1.8-abs(iMusic[3].x - iMusic[3].y)); // Contrast.
    vec3 col = firePalette(c); // Palettization.
    col = mix(col, col.zyx*0.1+c*(0.8 + 0.2*length(iMusic[0].xz) ), (1.+rd.x+rd.y)*0.45 ); // Color dispersion.
