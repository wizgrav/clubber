...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
	...
	if(dt<0.005){
	    ...
	    float atten = min((0.5+iMusic[2].z)/(distlpsp) + (0.5+iMusic[3].z)/(distlpsp2), 1.);
    	
    	// Ambient light.
	    float ambience = 0.1 + min(length(iMusic[3].xz),length(iMusic[0].xz))/3.;
    	...
    	// Specular lighting.
	    float spec = pow(max( dot( reflect(-ld, sn), -rd ), 0.0 ), 4. + 8. * iMusic[2].z);
	    float spec2 = pow(max( dot( reflect(-ld2, sn), -rd ), 0.0 ), 4. + 8. * iMusic[3].z);
    	
    	// Curvature.
	    float crv = clamp(curve(sp, 0.125)*0.5+0.5, .0, 1.);
	    
	    // Fresnel term. Good for giving a surface a bit of a reflective glow.
        float fre = pow( clamp(dot(sn, rd) + 1., .0, 1.), 2.+iMusic[0].x);
        ...
        vec3 texCol;
        if (sp.y<-(FH-0.005)) texCol = vec3(0.1 + dot(iMusic[0].yw,iMusic[3].yw )); // Floor.
 	    else texCol = vec3(0.33); // Walls.
        ...
        sceneCol *= clamp(1.-abs(curve(sp, 0.005)), .0, 1.);        
